      ...
      <CardBody className="text-center">
        ...
        {colors.map((color, index) => (
          <React.Fragment key={index}>
            <Button color={color.value} onClick={() => toggle(index)} className="mr-1">
              {color.name}
            </Button>
            <Modal isOpen={openModals[index]} toggle={() => toggle(index)}>
              <ModalHeader toggle={() => toggle(index)}>Default modal</ModalHeader>
              ...
              <ModalFooter>
                ...
                <Button color={color.value} onClick={() => toggle(index)}>
                  Save changes
                </Button>
              </ModalFooter>
            </Modal>
      ...
      <CardBody className="text-center">
        ...
        {colors.map((color, index) => (
          <React.Fragment key={index}>
            ...
            >
              <ModalHeader toggle={() => toggle(index)}>Colored modal</ModalHeader>
              <ModalBody className="text-center m-3">
                <p className="mb-0">
                  Use Bootstrap’s JavaScript modal plugin to add dialogs to your site for lightboxes, user
                  notifications, or completely custom content.
      ...
      <CardBody className="text-center">
        ...
        {colors.map((color, index) => (
          <React.Fragment key={index}>
            ...
            <Modal isOpen={openModals[index]} toggle={() => toggle(index)} centered>
              <ModalHeader toggle={() => toggle(index)}>Centered modal
      ...
      <CardBody className="text-center">
        ...
        {sizes.map((size, index) => (
          <React.Fragment key={index}>
            ...
            <Modal isOpen={openModals[index]} toggle={() => toggle(index)} size={size.value}>
              <ModalHeader toggle={() => toggle(index)}>{size.name} modal</ModalHeader>
              <ModalBody className="text-center m-3">
                <p className="mb-0">
                  Use Bootstrap’s JavaScript modal plugin to add dialogs to your site for lightboxes, user
                  notifications, or completely custom content.
