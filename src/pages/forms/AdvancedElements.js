    ...
    <CardBody>
      <Row>
        <Col lg="6">
          <FormGroup>
            <Label>Date</Label>
            <InputMask mask="99/99/9999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
          ...
          <FormGroup>
            ...
            <InputMask mask="99/99/9999 99:99:99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "DD/MM/YYYY HH:MM:SS"</span>
          </FormGroup>

          <FormGroup>
            <Label>ZIP Code</Label>
            <InputMask mask="99999-999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "DD/MM/YYYY HH:MM:SS"</span>
          </FormGroup>

          <FormGroup>
            <Label>ZIP Code</Label>
            <InputMask mask="9-99-99-99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "x-xx-xx-xx"</span>
          </FormGroup>

          <FormGroup>
            <Label>Money</Label>
            <InputMask mask="999.999.999.999.999,99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "Your money"</span>
          </FormGroup>

          <FormGroup>
            <Label>Money 2</Label>
            <InputMask mask="9.999,99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
        ...
        <Col lg="6">
          <FormGroup>
            ...
            <InputMask mask="9999-9999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "xxxx-xxxx"</span>
          </FormGroup>

          <FormGroup>
            <Label>Telephone with Code Area</Label>
            <InputMask mask="(99) 9999-9999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "(xx) xxxx-xxxx"</span>
          </FormGroup>

          <FormGroup>
            <Label>US Telephone</Label>
            <InputMask mask="(999) 999-9999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "(xxx) xxx-xxxx"</span>
          </FormGroup>

          <FormGroup>
            <Label>São Paulo Celphones</Label>
            <InputMask mask="(99) 99999-9999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "(xx) xxxxx-xxxx"</span>
          </FormGroup>

          <FormGroup>
            <Label>CPF</Label>
            <InputMask mask="999.999.999-99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "xxx.xxx.xxxx-xx"</span>
          </FormGroup>

          <FormGroup>
            <Label>CNPJ</Label>
            <InputMask mask="99.999.999/9999-99">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
            <span className="text-muted">e.g "xx.xxx.xxx/xxxx-xx"</span>
          </FormGroup>

          <FormGroup>
            <Label>IP Address</Label>
            <InputMask mask="999.999.999.999">{(inputProps) => <Input {...inputProps} type="text" />}</InputMask>
